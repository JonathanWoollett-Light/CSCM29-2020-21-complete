...
/**
 * TxInputList
 * defines a list of TxInputs  of a  transaction
 */
...
public class TxInputList {
    ...
    // If we have a Wallet covering the sender and an txOutputList then
    //  we can compute the signature by signing the transaction to be signed
    //  consisting of the public key and input amount and the txOutputList
    ...
    //  note that it is not verified that the signatures are correct
    public TxInputList(PublicKey sender1, int amount1, byte[] signature1,
    ...
    public TxInputList(String sender1, int amount1,
                       String sender2, int amount2,
                       TxOutputList txOutputList, Wallet wallet)
            throws NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        txInputList = new ArrayList<TxInput>();
    ...
    public TxInputList(PublicKey sender1, int amount1,
    ...
    public static void test()
        ...
        (new TxInputList(pubKeyA, 10, signedMessage1,
                pubKeyA, 10, signedMessage1)).testCase("Alice twice 10", pubKeyMap);

        TxInputList l = new TxInputList(pubKeyA, 10, signedMessage1,
   ...
   // main function running test cases
    ...
    /**
     * function  to print all items in the TxInputList
     * in the form
    ...
    public boolean checkSignature(TxOutputList txol) {
        for (TxInput entry : this.txInputList) {
            if (!entry.checkSignature(txol)) return false;
        }
        return true;
     ...
     * printing out the TxInputList
     * and printing out the sum of amounts in the TxInputList
     */
